...
describe("useFlowStore removeNode clears pins (4.4)", () => {
  ...
  it("removing a node also removes its pins; undo restores the node without pin resurrection", () => {
    ...
    // Undo restores the node. Pins are frontend-only (D2) and scoped to the
    // node's lifetime: deleting the node discards them, so undo does NOT
    // resurrect pin state (the flow history only snapshots nodes/edges).
