...
| Enrichment Nodes (inbetween-generation, ENRICHMENT category) | Malformed params / unbounded work | Low | Param validation via node `validate()` before execution (`interpolation_method`, `easing`, `euler_filter`, `tangent_smoothing∈[0,1]`, `target_fps>0`); static allowlist registration (SDD §4.3); pure in-memory stdlib math, no IO; `asyncio.to_thread` offload keeps the event loop responsive. | Invalid-param rejection tests; registry citizenship tests; DAG connection validation. |
| Retarget Presets (retarget-map, RIGGING category) | Malicious preset file / preset poisoning | High | Preset ingestion allowlist: relative paths resolved under `media/presets` only (absolute, `..`, symlink escapes rejected), extensions `.yaml`/`.yml`/`.json`; 262 KiB stat cap enforced strictly before parsing (billion-laughs defense); `yaml.safe_load`/`json.load` only — no `eval`/`exec`/unsafe loaders; Pydantic `extra="forbid"` validation plus unknown-bone rejection against the neutral skeleton. | Traversal / oversize / malformed-YAML rejection tests; static no-eval source scan; unknown-field and unknown-bone validation rejection tests. |
| Blocking Payload (blocking-input node, SOURCE category) | Malformed / oversized / hostile payload — unbounded parse cost, code injection | Medium | Payload validated at the adapter boundary: Pydantic `extra="forbid"` with `MAX_KEYPOSES=1000` count cap, quaternion unit-norm + finite, translation/scale finite, weight ∈ [0, 1]; `MAX_BLOCKING_PAYLOAD_CHARS` (4M chars) size cap enforced before any `json.loads`; no `eval`/`exec` anywhere in the module; pure in-memory stdlib math, no IO; `asyncio.to_thread` offload keeps the event loop responsive. | Oversize / non-finite / non-unit-quaternion / >`MAX_KEYPOSES` rejection tests; static no-eval source scan; E2E executor graph test (`blocking-input → inbetween-generation`). |
