...
class CoreClient:
    ...
    def poll_pending(self, session_id: str) -> dict[str, object] | None:
        """Poll for pending payloads from the Core.

        ``GET /sessions/{id}/pending``. Returns the payload dict if available,
        or ``None`` if no pending payloads (204 response).

        Args:
            session_id: The registered session ID.

        Returns:
            The pending payload dict, or ``None`` if empty.
        """
        try:
            status, body = self._transport.request(
                "GET", f"/sessions/{session_id}/pending", token=self._token
            )
            if status == 204:
                return None
            return body
    ...
    def submit_edited_poses(
        ...
        *,
        ...
        """Submit edited poses back to the Core.

        ``POST /sessions/{id}/submit_edited_poses``. Sends the NeutralMotion
        document with edited transforms after the animator finishes pose-mode
        editing in Blender.

        Args:
            session_id: The registered session ID.
            motion: The NeutralMotion document (as a dict) with edited transforms.
            source_frame_range: Optional frame range for partial delivery.
            request_id: Optional request ID for correlation.

        Returns:
            True if accepted (202), False on error.
        """
        payload: dict[str, object] = {
            "kind": "edited_poses",
            "motion": motion,
        ...
        if source_frame_range is not None:
            payload["source_frame_range"] = list(source_frame_range)
        if request_id:
            payload["request_id"] = request_id
        try:
            status, _ = self._transport.request(
                "POST", f"/sessions/{session_id}/submit_edited_poses",
