...
class TestInbetweenGenerationNodeExecute:
    ...
    async def test_execute_returns_enriched_neutral_motion(self, context: ExecutionContext) -> None:
        """Should return an enriched NeutralMotion (upsampled to target_fps)."""
    ...
    @pytest.mark.asyncio
    async def test_execute_passthrough_without_upsample(self, context: ExecutionContext) -> None:
        """Should pass through unchanged when target_fps <= meta.fps."""
        node = InbetweenGenerationNode()
        result = await node.execute(
            inputs={"motion": _motion()},
            params={"target_fps": 24.0, "easing": "ease-in"},
            context=context,
        )
        motion = result.values["motion"]
        assert isinstance(motion, NeutralMotion)
        # No resample: the 2 source frames survive at their original rate.
        assert [f.frame for f in motion.frames] == [1, 2]
        assert motion.meta.fps == 24.0
...
class TestInbetweenGenerationNodeValidate:
    ...
    are OK (defaults apply) and invalid enum/range/fps/bool values are
