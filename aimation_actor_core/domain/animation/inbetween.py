...
- :func:`_resample` — keyframe-exact cubic/linear upsample with easing fused
  into the per-interval timing, upsample-only; rotations are interpolated
  with shortest-arc slerp (RESAMPLE / ROT).
...
- :func:`_apply_rotation_filter` — running-sign canonicalization of each
  joint's rotation track (ROT).
- :func:`_apply_tangent_smooth` — centered-box smoothing on translation
  axes only (SMOOTH).
...
def _interp_value(
    ...
    return _hermite_value(values[index], tangents[index], values[index + 1], tangents[index + 1], u)
...
def _resample(
    ...
    tracks = {bone: [f.pose.transforms[bone].translation for f in motion.frames] for bone in bones}
    tangents = {
...
def _nlerp(
    ...
    The anti-NaN fallback for slerp: it divides by nothing and re-normalizes,
    so it stays finite for near-parallel (or near-antipodal, pre-flip) pairs.
    """
    blend = (
        q0[0] + t * (q1[0] - q0[0]),
        q0[1] + t * (q1[1] - q0[1]),
...
def _apply_tangent_smooth(motion: NeutralMotion, intensity: float) -> NeutralMotion:
    ...
    The box window is ``1 + round(intensity * 9)`` (even results are rounded
    down to the nearest odd width, so the box stays perfectly centered):
    ``intensity=0`` is the identity (window 1) and the smoothing magnitude is
    non-decreasing with the parameter. Every output sample is the mean of
    exactly ``window`` inbound samples (the window is bounded by repeating the
    edge value), so the filter is variance-reducing on the trajectory's jitter.
    The filter is applied per translation axis only — rotations are left
    untouched (norm-drift risk; explicit design decision). Returns ``motion``
    unchanged when ``intensity <= 0``.
    """
    if intensity <= 0.0:
        return motion
    ...
    for bone in bones:
        ...
        axes = ([t[0] for t in track], [t[1] for t in track], [t[2] for t in track])
        smoothed[bone] = [
            (_box1d(axes[0], i), _box1d(axes[1], i), _box1d(axes[2], i)) for i in range(n_frames)
        ]
