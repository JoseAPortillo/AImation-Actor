...
- [x] 6.3 `npm test` in `frontend/` green — fixture no drift (note: 2 pre-existing App.test.tsx failures at HEAD, unrelated — see apply-progress)
- [x] 6.4 Chain `video-source→pose-2d→pose-3d→video-to-motion→temporal-cleanup→inbetween-generation` valid DAG (SEED)
- [x] 6.5 §3.2: threat-model entry + Security Champion sign-off before archive (entry added; sign-off pending at archive)
