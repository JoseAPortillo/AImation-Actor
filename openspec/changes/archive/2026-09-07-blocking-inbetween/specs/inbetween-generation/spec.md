# inbetween-generation — Delta Specs

== Capability: inbetween-generation (MODIFIED) ==
...
- The former constraint "MVP leaves `contacts`/`keyposes` untouched (frame references may become stale after resampling; remapping is future work)" is REMOVED: this change ships keypose frame-index remapping and key-lock (REQ-05, REQ-06, REQ-07). `contacts` and `tracking` remain passthrough/unmodified.
- `domain/animation/inbetween.py` MUST stay pure stdlib (`math` only): no scipy, no numpy — unchanged.
